        ...
        <div class="page-breadcrumb bg-white">
            <div class="d-flex justify-content-between align-items-center">
                <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                    <h4 class="page-title">SỬA KHUYẾN MÃI</h4>
                </div>
            </div>
            <!-- /.col-lg-12 -->
        </div>
                    ...
                    <div class="form-group">
                        <form action="{{route('admin.discount.update', [$discount->id])}}" method="post"
                            ...
                            <div class="mt-2" style="margin-bottom:20px; display: flex; align-items:center;">
                                <label style="display: inline-block; margin-bottom: 0px;">Giảm giá theo phần trăm:</label>
                                <input style="margin-left: 10px;" type="checkbox" name="is_percent" @php echo $discount->is_percent ? "checked" : "" @endphp>
                                @error('percent')
                                <span class="text-danger">{{$message}}</span>
                                @enderror
                            </div>
                            <div class="mt-2" style="margin-bottom:20px">
                                ...
                                <input type="text" class="form-control" name="val" value="{{$discount->val}}">
                                @error('val')
                                <span class="text-danger">{{$message}}</span>
                                @enderror
                            </div>
